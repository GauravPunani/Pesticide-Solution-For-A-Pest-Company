...
$total_rows = $wpdb->get_var("
    select count(*)
    from {$wpdb->prefix}vehicle_breakpads_change_proof VPP
    left join {$wpdb->prefix}technician_details TD
    on VPP.technician_id=TD.id
    join {$wpdb->prefix}vehicles V
    on VPP.vehicle_id=V.id
...
$records = $wpdb->get_results("
    select VPP.*, TD.first_name, TD.last_name, V.year, V.make, V.model, L.slug as branch_name
    from {$wpdb->prefix}vehicle_breakpads_change_proof VPP
    left join {$wpdb->prefix}technician_details TD
    on VPP.technician_id=TD.id
    join {$wpdb->prefix}vehicles V
    ...
    <div class="row">
        ...
        <div class="col-sm-12">
            <div class="card full_width table-responsive">
                <div class="card-body">
                    <h3 class="card-title">Break Pads Change Proof <small>(<?= $total_rows; ?> Records Found)</small></h3>
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                                <th>Vehicle</th>
                                <th>Assigned to</th>
                                <th>Break Pads Change Mileage</th>
                                <th>Mileage Proof</th>
                                <th>Break Pads Change Proof</th>
                                <th>Branch</th>
                                <th>Date Created</th>
                                <th>Action</th>
                            </tr>
                            ...
                            <?php if (is_array($records) && count($records) > 0) : ?>
                                <?php foreach ($records as $record) : ?>
                                    <tr>
                                        ...
                                        <!-- MILEAGE PROOF  -->
                                        <?php if (!empty($record->mileage_proof)) : ?>
                                            <td><a target="_blank" class="btn btn-primary" href="<?= $record->mileage_proof;  ?>"><span><i class="fa fa-eye"></i></span> View</a></td>
                                        <?php else : ?>
                                            <td><b>Not Found</b></td>
                                        <?php endif; ?>

                                        <!-- OIL CHANGE PROOF  -->
                                        <?php if (!empty($record->breadkpad_proof)) : ?>
                                            <td><a target="_blank" class="btn btn-primary" href="<?= $record->breadkpad_proof;  ?>"><span><i class="fa fa-eye"></i></span> View</a></td>
                                        <?php else : ?>
                                            <td><b>Not Found</b></td>
                                        <?php endif; ?>
    ...
    (function($) {
        $('.approve_reject_mileage_proof').on('click', function() {
            ...
            $.ajax({
                ...
                data: {
                    action: "approve_reject_break_pads_change_proof",
                    status: status,
                    mileage: mileage,
                    proof_id: proof_id,
                    "_wpnonce": "<?= wp_create_nonce('approve_reject_break_pads_change_proof'); ?>"
                },
