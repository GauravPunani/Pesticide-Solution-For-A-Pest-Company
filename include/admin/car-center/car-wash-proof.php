...
$total_rows = $wpdb->get_var("
    select count(*)  
    from {$wpdb->prefix}car_wash_proof VI
    ...
    <div class="row">
        ...
        <div class="col-sm-12">
            <div class="card full_width table-responsive">
                <div class="card-body">
                    <h3 class="page-header">Car Wash Proof <small>(<?= $total_rows; ?> Records Found)</small></h3>
                            ...
                            <?php if (is_array($records) && count($records) > 0) : ?>
                                <?php foreach ($records as $record) : ?>
                                    <tr>
                                        ...
                                        <td><button class="btn btn-primary show_proof_of_wash" data-car-wash-proof='<?= $record->proof_of_wash; ?>'><span><i class="fa fa-eye"></i></span> Show</button></td>
                                        <td><?= (new GamFunctions)->beautify_string($record->branch_name); ?></td>
                                        <td><?= !empty($record->created_at) ? date('d M Y h:i A', strtotime($record->created_at)) : ''; ?></td>
                                        <td>
...
<!-- Modal -->
<div id="car_wash_proof_docs" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Car Wash Proofs</h4>
            </div>
...
<script>
    ...
    function approveConditionProof(proof_id, ref) {
        jQuery.ajax({
            ...
            dataType: "json",
            data: {
                action: "approve_reject_car_wash_proof",
                status: "approve",
                proof_id,
                "_wpnonce": "<?= wp_create_nonce('approve_reject_car_wash_proof'); ?>"
            },
            beforeSend: function() {
            ...
            success: function(data) {
                if (data.status == "success") {
                    ...
                    jQuery(ref).prop('checked', true);
                } else {
                    new swal('Opps!', data.message, 'error');
    ...
    (function($) {
        $(document).ready(function() {
            ...
            $('.show_proof_of_wash').on('click', function() {

                const proof_data = $(this).attr('data-car-wash-proof');
                const proof_html = generateDocsHtml(proof_data);

                $('.proof_body').html(proof_html);
                $('#car_wash_proof_docs').modal('show');
