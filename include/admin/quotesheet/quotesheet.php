...
if (!empty($_GET['edit-id'])) {
    return get_template_part('/include/admin/quotesheet/edit-residential-quote', null, ['data' => $_GET['edit-id']]);
}

if (!empty($_GET['quote_id'])) {
    return get_template_part('/include/admin/quotesheet/view-residential-quote', null, ['data' => $_GET['quote_id']]);
}
...
if (!empty($_GET['branch_id']) && $_GET['branch_id'] != "all") {
    $conditions[] = " Q.branch_id = '{$_GET['branch_id']}'";
}
...
if (!empty($_GET['lead_type'])) {
    ...
    switch ($_GET['lead_type']) {
        ...
        case 'dead':
        case 'closed':
            $conditions[] = " Q.lead_status='{$_GET['lead_type']}'";
            break;
    }
}
...
if (!empty($_GET['search'])) {
    ...
    if (!empty($conditions)) {
        $conditions .= " " . (new GamFunctions)->create_search_query_string($whereSearch, $_GET['search'], 'and', 'Q');
    } else {
        $conditions = (new GamFunctions)->create_search_query_string($whereSearch, $_GET['search'], '', 'Q');
    }
}
        ...
        <div class="col-sm-12">
            <div class="card full_width table-responsive">
                <div class="card-body">
                    ...
                    <h3 class="page-header">Residential Quotes</h3>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" data-toggle="buttons">
                                                <button data-lead-status="pending" class="btn <?= $quote->lead_status == "pending"  ? 'btn-primary' : 'btn-default'; ?> lead-<?= $quote->id; ?> lead-status" data-lead-id="<?= $quote->id; ?>">Pending</button>
                                                <button data-lead-status="dead" class="btn <?= $quote->lead_status == "dead"  ? 'btn-primary' : 'btn-default'; ?> lead-<?= $quote->id; ?> lead-status" data-lead-id="<?= $quote->id; ?>">Dead</button>
                                                <button data-lead-status="closed" class="btn <?= $quote->lead_status == "closed"  ? 'btn-primary' : 'btn-default'; ?> lead-<?= $quote->id; ?> lead-status" data-lead-id="<?= $quote->id; ?>">Closed</button>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <ul class="custom-dropdown dropdown-menu dropdown-menu-left">
                                                    ...
                                                    <li><a onclick="downloadResidentialQuote(<?= $quote->id; ?>)" href="javascript:void(0)"><span><i class="fa fa-download"></i></span> Download</a></li>

                                                    <li><a target="_blank" href="<?= $_SERVER['REQUEST_URI']; ?>&edit-id=<?= $quote->id; ?>"><span><i class="fa fa-edit"></i></span> Edit Quote</a></li>

                                                    <?php if (!empty($quote->clientPhn) || !empty($quote->clientEmail)) : ?>

                                                        <?php if (!empty($quote->clientEmail)) : ?>
                                                            <li><a onclick="smsQuoteLink(<?= $quote->id; ?>, '<?= $quote->clientPhn; ?>')" href="javascript:void(0)"><span><i class="fa fa-envelope"></i></span> SMS Link</a></li>
                                                        <?php else : ?>
                                                            <li class="disabled"><a href="javascript:void(0)"><span><i class="fa fa-envelope"></i></span> SMS Link (<i>No Email</i>)</a></li>
                                                        <?php endif; ?>
