...
if(!empty($_GET['technician_id'])) $conditions[] = " SPN.technician_id = '{$_GET['technician_id']}'";
if(!empty($_GET['date'])) $conditions[] = " date(SPN.date) = '{$_GET['date']}'";

if(!empty($_GET['search'])){
    $whereSearch=(new GamFunctions)->get_table_coloumn($wpdb->prefix.'special_notes');
    $conditions[] =(new GamFunctions)->create_search_query_string($whereSearch,$_GET['search'],'no_type', 'SPN');
}

$conditions = count($conditions) > 0 ? (new GamFunctions)->generate_query($conditions) : '';

$notes = $wpdb->get_results("
    select SPN.*, TD.first_name, TD.last_name
    from {$wpdb->prefix}special_notes SPN

    left join {$wpdb->prefix}technician_details TD
    on TD.id = SPN.technician_id
