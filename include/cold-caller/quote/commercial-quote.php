<?php
    ...
    if(isset($_GET['quote_id']) && !empty($_GET['quote_id'])){
        require_once "commercial-quote-details.php";
        return;
    }
    ...
    $total_pages_sql = "select COUNT(*) from  {$wpdb->prefix}commercial_quotesheet $whereSearch";
    
    $total_rows= $wpdb->get_var($total_pages_sql);
    
    $total_pages = ceil($total_rows / $no_of_records_per_page);
    
    $result = $wpdb->get_results("select * from {$wpdb->prefix}commercial_quotesheet $whereSearch order by date DESC LIMIT $offset, $no_of_records_per_page ");

?>
<div class="table-responsive">

    <h3 class="text-center">Commercial Quotes</h3>
