...
class DownloadCriteriaReportWithSelector
{
    ...
    function __construct(){
        $this->config_file_path=get_template_directory()."/config_files/google key/adsapi_php.ini";
    }
    ...
    public function runExample(AdWordsSession $session, $filePath,$date){
        ...
        unset($daily_spent[0],$daily_spent[1]);
        array_pop($daily_spent);
        $daily_spent=array_values($daily_spent);

        $daily_spent=$this->group_campaign_amounts($daily_spent);

        // search for campaign name name in db and search for last 7 days records for that in callrail 
        $unknown_spends = [];

        foreach($daily_spent as $key=>$val){
            ...
            if($result){
                ...
                $total_calls=(new Callrail_new)->get_calls_by_tracking_no($result->tracking_phone_no,$result->location,'','','yesterday');
    ...
    public function main($date){
        ...
        // check if for the given date , data is already fetched
        $res = $wpdb->get_var("
            select count(*) from 
            {$wpdb->prefix}googleads_daily_data 
            where date='$date' 
            and account_name='pest_control'
        ");

        if($res){
            $comment="google ac1 daily data is already fetched for the date $date";
        ...
        $filePath = sprintf(
            '%s.csv',
            tempnam(sys_get_temp_dir(), 'capmpaign-report-')
        );
