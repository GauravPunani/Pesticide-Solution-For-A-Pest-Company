<?php
    ...
    $total_pages_sql = "select COUNT(*) from {$wpdb->prefix}reimbursement_proof where $where $search_query and status='paid' order by created_at desc";
    ...
    $data=$wpdb->get_results("select * from {$wpdb->prefix}reimbursement_proof where $where $search_query and status='paid' order by created_at desc");
?>

<div class="row">

    <?php if(!empty($search_query)): ?>
        <p>Total Records Found : <b><?= $total_rows; ?></b> <a href="<?= strtok($_SERVER["REQUEST_URI"], '?').'?view=reimbursed'; ?>"><span><i class="fa fa-database"></i></span> Show All</a></p>
    <?php endif; ?>
